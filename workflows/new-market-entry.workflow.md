...
description: Enrich top companies to understand industry patterns, pain points, and buying behaviors. Run this for the top 3 companies from Step 2 and collect the results under company_profiles.
...
When a Motion is created (typically via the UI) it auto-creates a Default Motion Playbook covering the persona × segment matrix as Motion ICP cells. For a market-entry deliverable, create a Custom Motion Playbook with narrative type `THEMATIC` (general angle), `ACCOUNT` (account-focused), or `COMPETITIVE` (displacement) — pick the narrativeType that matches the entry intent.

tool: list_motions
save_as: available_motions
description: List Motions and ask the user which Motion this Custom Motion Playbook should be layered on. Save the chosen Motion's oId as target_motion_oId. If no Motion exists yet for the offering, ask the user to create one first (creating a Motion auto-generates the Default Motion Playbook with the persona × segment matrix).
...
description: Build a Custom Motion Playbook for this market.
