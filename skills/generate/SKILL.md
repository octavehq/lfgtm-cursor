---
name: generate
description: Generate GTM content (emails, LinkedIn messages, call prep) using saved agents, Octave AI, or the current assistant directly — your choice. Use when user says "generate an email", "write a LinkedIn message", "prep for a call", "create outreach", or asks for single-asset content generation with mode selection.
---
...
Honor the named engine/agent; otherwise default to Octave native generation. Assistant Direct means the current assistant. Resolve channel, count, language, audience, offering, segment and playbook once. A singular email means one email. Use the same content packet and factual QA for every engine.
...
- [content contracts](references/content-contracts.md)
