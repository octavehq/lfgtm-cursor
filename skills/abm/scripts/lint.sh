#!/bin/bash
# ABM mechanical lint — deterministic checks that don't need LLM judgment.
# Usage: bash lint.sh <path-to-account-plan.html>
# Returns non-zero exit code if any violations found.

FILE="$1"
if [ -z "$FILE" ] || [ ! -f "$FILE" ]; then
  echo "Usage: bash lint.sh <path-to-account-plan.html>"
  exit 1
fi
