...
# /octave:product-launch - Launch Command Center
...
- [Entity model](../shared/entity-model.md): canonical entity types and oId prefixes for products, personas, Motions, Motion Playbooks, and Motion ICPs

## Usage

```
/octave:product-launch [description] [--type product|feature|update|partnership|expansion] [--persona <name>]
```

## Examples

```
/octave:product-launch                                            # Interactive mode
/octave:product-launch "New AI analytics feature"                 # Launch a feature
/octave:product-launch "Enterprise tier" --type product           # New product launch
...
When the user runs `/octave:product-launch`:
...
- **Saved agents**: Check for matching agents with `list_agents` when relevant. Browse them with the `list_agents` MCP tool.
...
- `/octave:positioning` - Build messaging framework before the launch
- `/octave:generate` - Ongoing content post-launch
- `/octave:one-pager` / `/octave:deck` - Deep-dive on specific collateral
- `/octave:battlecard-doc` - Update competitive positioning for the launch
- `/octave:train` - Team training for the launch
