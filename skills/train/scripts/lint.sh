#!/bin/bash
# Onboarding GTM primer mechanical lint — deterministic checks that don't need LLM judgment.
# Usage: bash lint.sh <path-to-primer.html>
# Returns non-zero exit code if any violations found.

FILE="$1"
if [ -z "$FILE" ] || [ ! -f "$FILE" ]; then
  echo "Usage: bash lint.sh <path-to-primer.html>"
  exit 1
fi
