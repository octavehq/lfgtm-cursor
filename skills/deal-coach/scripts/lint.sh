#!/bin/bash
# Deal-coach mechanical lint — deterministic checks that don't need LLM judgment.
# Usage: bash lint.sh <path-to-coaching.html>
# Returns non-zero exit code if any violations found.

FILE="$1"
if [ -z "$FILE" ] || [ ! -f "$FILE" ]; then
  echo "Usage: bash lint.sh <path-to-coaching.html>"
  exit 1
fi
